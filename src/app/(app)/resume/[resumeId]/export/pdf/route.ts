...
import { renderResumePdf } from "@/features/resume/export/pdf";
import {
  RESUME_TEMPLATE_IDS,
...
export async function GET(
  ...
  const templateId: ResumeTemplateId = RESUME_TEMPLATE_IDS.includes(
    requestedTemplate as ResumeTemplateId,
  )
    ? (requestedTemplate as ResumeTemplateId)
    : "minimal";

  const buffer = await renderResumePdf(templateId, data);

  await consumeEntitlement(user.id, "RESUME_EXPORT");
  await logAuditEvent("resume.exported", {
    userId: user.id,
    metadata: { resumeId, format: "pdf", templateId },
  });

  return new NextResponse(new Uint8Array(buffer), {
    headers: {
      "Content-Type": "application/pdf",
      "Content-Disposition": `attachment; filename="${resume.title.replace(/[^a-z0-9-_ ]/gi, "")}.pdf"`,
    },
  });
}
